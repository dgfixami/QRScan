document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add helper function to sanitize text inputs
    ...
    // Add helper function to create safe DOM elements
    ...
    // Function to log messages to the page - secure version
    ...
    // Show lookup error - secure version
    function showLookupError(message) {
        lookupResult.innerHTML = '';
        
        const errorDiv = createSafeElement('div', {className: 'lookup-error'});
        const errorP = createSafeElement('p');
        errorP.textContent = `⚠️ ${sanitizeInput(message)}`;
        errorDiv.appendChild(errorP);
        
        lookupResult.appendChild(errorDiv);
        lookupResult.classList.remove('hidden');
    }
    ...
    // Modified function to send data to Google Sheets with CSRF protection
    function sendToGoogleSheets(scanData, callback) {
        ...
        const requestData = {
            ...scanData,
            csrf: localStorage.getItem('qrscan_csrf_token')
        };
